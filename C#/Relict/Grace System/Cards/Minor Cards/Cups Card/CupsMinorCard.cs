...
public class CupsMinorCard : MinorCardBase
{
    ...
    [Tooltip("Expecting one modifier to apply to Max HP")]
    ...
    [Tooltip("Expecting two modifiers. Element 0 will apply to Max HP and Element 1 will apply to Healing Regen")]
    ...
    [Tooltip("Expecting three modifiers. Element 0 will apply to Max HP, Element 1 will apply to Healing Regen, and Element 2 will apply to Heal On Kill")]
    ...
    private void Start()
    {
        ...
        playerStats.MaxHP.AddModifier(newStatModOne);
    }
    ...
    public override string GetStats()
    {
        ...
        float maxHPValue = Mathf.Abs(playerStats.MaxHP.Value - playerStats.MaxHP.BaseValue);
        if (maxHPValue != 0f) 
        {
            result += "Max HP Bonus: \n+" + maxHPValue.ToString("0.00") + " \n\n";
        }

        float healthRegenValue = Mathf.Abs(playerStats.HealthRegeneration.Value - playerStats.HealthRegeneration.BaseValue);
        if (healthRegenValue != 0f) 
        {
            result += "Health Regen Bonus: \n+" + healthRegenValue.ToString("0.00") + " \n\n";
        }

        float healOnKillValue = Mathf.Abs(playerStats.HealthOnKill.Value - playerStats.HealthOnKill.BaseValue);
        if (healOnKillValue != 0f) 
        {
            result += "Heal On Kill Bonus: \n+" + healOnKillValue.ToString("0.00") + " ";
        }
    ...
    public override int Upgrade()
    {
        ...
        switch (currentCardTier)
        {
            ...
            case MinorCardTier.TierTwo:
                ...
                playerStats.MaxHP.RemoveAllModifiersFromSource(this);
                playerStats.MaxHP.AddModifier(newStatModOne);
                break;
            ...
            case MinorCardTier.TierFive:
                ...
                playerStats.HealthRegeneration.RemoveAllModifiersFromSource(this);
                playerStats.HealthRegeneration.AddModifier(newStatModTwo);
                break;
            ...
            case MinorCardTier.TierTen:
                ...
                playerStats.HealthOnKill.RemoveAllModifiersFromSource(this);
                playerStats.HealthOnKill.AddModifier(newStatModThree);
                break;
